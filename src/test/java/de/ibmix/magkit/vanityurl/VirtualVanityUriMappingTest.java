...
public class VirtualVanityUriMappingTest {
    ...
    public void testExtractSiteFromUrl() {
        VirtualVanityUriMapping.SiteUrlInfo result = _uriMapping.extractSiteFromUrl("/Dotmar/test");
        assertThat(result.getSiteName(), equalTo("Dotmar"));
        assertThat(result.getVanityUrl(), equalTo("/test"));
        ...
        assertThat(result.getSiteName(), equalTo((String) null));
        assertThat(result.getVanityUrl(), equalTo("/NonExistentSite/test"));
    }
    ...
    @Test
    public void testSiteSpecificVanityUrl() throws Exception {
    ...
    @Test
    public void testSiteSpecificVanityUrlWithNonExistentSite() throws Exception {
    ...
    public void setUp() {
        ...
        // Mock site-specific vanity URL
        MockNode siteSpecificNode = new MockNode("test");
        when(vanityUrlService.queryForVanityUrlNode("/test", "Dotmar")).thenReturn(siteSpecificNode);
        when(vanityUrlService.queryForVanityUrlNode("/test", NullSite.SITE_NAME)).thenReturn(null);

        when(vanityUrlService.createRedirectUrl(mockNode, null)).thenReturn("redirect:/internal/page.html");
        when(vanityUrlService.createRedirectUrl(mockNode, null, null)).thenReturn("redirect:/internal/page.html");
        when(vanityUrlService.createRedirectUrl(siteSpecificNode, null)).thenReturn("redirect:/products.html");
        when(vanityUrlService.createRedirectUrl(siteSpecificNode, null, null)).thenReturn("redirect:/products.html");
