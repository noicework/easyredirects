...
public class VanityUrlServiceTest {
    ...
    public void testWildcardPatternMatching() {
        ...
        Map<String, String> result = VanityUrlService.matchPattern("/products/electronics/laptop", "/products/*");
        assertThat(result, notNullValue());
        assertThat(result.isEmpty(), equalTo(true));

        // Test no match
        result = VanityUrlService.matchPattern("/blog/2023/article", "/products/*");
        assertThat(result, nullValue());

        // Test wildcard in middle
        result = VanityUrlService.matchPattern("/docs/v1/latest", "/docs/*/latest");
        assertThat(result, notNullValue());
    }
    ...
    public void testPathParameterMatching() {
        ...
        Map<String, String> result = VanityUrlService.matchPattern("/user/john123/profile", "/user/{username}/profile");
        assertThat(result, notNullValue());
        assertThat(result.get("username"), equalTo("john123"));

        // Test multiple parameters
        result = VanityUrlService.matchPattern("/blog/2023/12", "/blog/{year}/{month}");
        assertThat(result, notNullValue());
        assertThat(result.get("year"), equalTo("2023"));
        assertThat(result.get("month"), equalTo("12"));

        // Test no match
        result = VanityUrlService.matchPattern("/blog/2023", "/blog/{year}/{month}");
        assertThat(result, nullValue());
    }
    ...
    public void testResolvePlaceholders() {
        ...
        String resolved = VanityUrlService.resolvePlaceholders("/archives/{year}-{month}/{slug}", params);
        assertThat(resolved, equalTo("/archives/2023-12/my-article"));

        // Test with missing parameter (should not replace)
        resolved = VanityUrlService.resolvePlaceholders("/archives/{year}/{missing}", params);
